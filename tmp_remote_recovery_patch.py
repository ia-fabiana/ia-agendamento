...
path = Path('/opt/ia-agendamento/server/index.mjs')
text = path.read_text(encoding='utf-8')

def replace_or_raise(src: str, dst: str, label: str):
    global text
    if src not in text:
        raise RuntimeError(f'pattern not found: {label}')
    text = text.replace(src, dst, 1)

helper_block = '''function extractIsoDateFromText(text, dateContext = getSaoPauloDateContext()) {
...
if 'function recoverBookingDraftFromHistory(' not in text:
    replace_or_raise(
        'function formatBookingItemSummary(item) {',
        helper_block + 'function formatBookingItemSummary(item) {',
        'insert recovery helpers'
    )

old_no_pending_block = '''  if (
...
new_no_pending_block = '''  const recoveredDraft =
...
replace_or_raise(old_no_pending_block, new_no_pending_block, 'replace no-pending confirmation flow')

path.write_text(text, encoding='utf-8')
print('remote_recovery_patch_applied')